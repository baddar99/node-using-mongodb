...
describe('Updating a product', () => {
  it('Add a controler to update a product @controller-update', done => {
    try {
      expect(fs.readFileSync('src/controllers/controllers.js').toString()).to
        .contain(`Product.findOneAndUpdate(
  ...
  it('Add a controler to update a product @route-update', done => {
    try {
      expect(fs.readFileSync('src/routes/routes.js').toString()).to.contain(
        '.put(updateProduct)'
      );
      done();
    } catch (error) {
